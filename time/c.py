...
def folder_path():
    return process_input_path(input("请输入目标路径："))

video_type = ["avi", "wmv", "mpeg", "mp4", "m4v", "mov", "flv", "rmvb"]

def get_video_paths(folder):
    return [os.path.join(path, file) for path, dirs, files in os.walk(folder) for file in files if file.split(".")[-1] in video_type]

def process_input_path(input_path):
    # 如果输入的路径是用反斜杠分隔的，就替换成正斜杠
    input_path = input_path.replace("\\", "/")
    # 如果输入的路径包含空格或中文，就用双引号包裹起来
    if " " in input_path or any(ord(c) > 127 for c in input_path):
...
def get_video_duration(file):
    ...
    duration = round(float(output) / 60, 3)
    return duration
